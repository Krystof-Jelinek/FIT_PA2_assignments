...
public:
	class CPerson
	{
		public:
            ...
            void killmeandmyfolowers(){
                if(m_Next){
                    m_Next->killmeandmyfolowers();
                }
                delete this;
            }
	};
    ...
    bool areFriends(const string & name1, const string & name2){
        CPerson * first = Find(name1);
        CPerson * second = Find(name2);

        for(CPerson * i : first->m_Friends){
            if(i == second){
                return true;
            }
        }
    ...
    CPerson * createChain(int position) const{
        CPerson * cur = m_Table[position];
        if(cur == nullptr){
            return nullptr;
        }
        CPerson * build = new CPerson(cur->m_Name);
        CPerson * result = build;
        cur = cur->m_Next;

        while(cur != nullptr){
            build->m_Next = new CPerson(cur->m_Name);
            build = build->m_Next;
            cur = cur->m_Next;
        }
        return result;
...
CNetwork::CNetwork ( const CNetwork & other )
{
	...
	for ( size_t i = 0; i < m_Size; i++ ){
        m_Table[i] = other.createChain(i);
    }

    for ( size_t i = 0; i < m_Size; i++ ){
        CPerson * cur = other.m_Table[i];
        while(cur){
            for(CPerson * ptr : cur->m_Friends){
                CPerson * new_table_ptr = Find(cur->m_Name);
                new_table_ptr->m_Friends.push_back(Find(ptr->m_Name));
            }
            cur = cur->m_Next;
        }
    }
