...
class CDisk : public CThing{
  ...
  string printMe(int indentation,vector<int> connections, bool last_in_list) const override{
    ...
    //TODO tady si nejsem uplne jistej je to nejaky shady
    int loop_counter = 0;
    ...
    for(const CPartition & i : partitions){
      ...
      ret += i.printMe(indentation,new_connections, true, counter);
      }
      else{
        ret += i.printMe(indentation,new_connections, false, counter);
      }
      counter++;
      loop_counter++;
    }
...
class CComputer : public CThing{
  ...
  string printMe(int indentation,vector<int> connections,bool last_in_list) const override{
    ...
    if(parts.empty()){
      ...
      for(const CAdress & j : addresses){
        if(cnt == int(addresses.size()) - 1){
          res += j.printMe(indentation + 2, connections, true);
          continue;
        }
        res += j.printMe(indentation +2, connections, false);
        cnt++;
      }
    }
    else{
      for(const CAdress & j : addresses){
        res += j.printMe(indentation +2, connections, false); //TODO tohle se snad uz nerozbije pokud bude mit pocitac pouze adresy
      }
    }
    ...
    for(const shared_ptr<CThing> & i : parts){
      if(counter == int(parts.size()) - 1){
        res += i->printMe(indentation, connections, true);
        continue;
      }
      res += i->printMe(indentation, new_connections, false);
      counter++;
    }
    return res;
  }
  ...
  friend ostream & operator << (ostream & ostream, const CComputer & in){
    string res;
    res = in.printMe(-2, {}, false);
    auto tmp = res.begin() + 2;
    res.erase(res.begin(), tmp);
    ostream << res;
    //ostream << &in;
    return ostream;
  }
...
int main ()
{
  CComputer r( "progtest.fit.cvut.cz" );
          r.addAddress ( "147.32.232.142" ) .
  ...
  cout << r;
  string jap = toString(r);
  string idk = "Host: progtest.fit.cvut.cz\n"
    "+-147.32.232.142\n"
    "+-CPU, 8 cores @ 2400MHz\n"
    "+-CPU, 8 cores @ 1200MHz\n"
    "+-Memory, 2000 MiB\n"
  ...
  CNetwork achjo ( "FIT network" );
  achjo.addComputer(CComputer ( "imap.fit.cvut.cz" ) .
          addAddress ( "147.32.232.238" ) .
          addAddress ( "2001:718:2:2901::238" )).addComputer (
